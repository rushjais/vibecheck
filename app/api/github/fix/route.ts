...
export async function POST(request: Request) {
  ...
  try {
    ...
    const newBranch = `launchguard/fix-${Date.now().toString(36)}`;
    ...
    for (const path of files) {
      ...
      await commitFile(
        ...
        file.sha,
        newBranch,
        `[LaunchGuard] Fix ${fileFindings.length} issue(s) in ${path}`,
      );
    ...
    const title = `[LaunchGuard] Fixed ${fixedCount} security ${fixedCount === 1 ? "issue" : "issues"}`;
...
function buildPrBody(changed: { path: string; findings: FindingRow[] }[]): string {
  ...
  const lines: string[] = [
    `## 🛡️ LaunchGuard auto-fix`,
    "",
    `This pull request fixes **${total} security ${total === 1 ? "issue" : "issues"}** found in your latest LaunchGuard scan.`,
    "",
    "### What changed",
    "",
  ];
  ...
  lines.push(
    "---",
    "",
    "These edits were generated by reading your code as text — **LaunchGuard never ran your code**. Please review the diff and let your usual CI and review process do the rest before merging.",
  );
  return lines.join("\n");
}
